...
oc get pods --no-headers=true $kubeconfig -A | grep -v -E 'Running|Completed' | grep -iE 'error|ContainerStatusUnknown|NodeAffinity' | while read line; do NS=$(echo "$line" | awk '{print $1}'); POD=$(echo "$line" | awk '{print $2}'); oc delete pod ${POD} -n ${NS} $kubeconfig; done
